...
import type { UnitInfo } from './data/unitsData';
...
const AcessoRapido = () => {
  ...
  const closestRegionName = useMemo(() => {
    if (!closestUnitId) return null;
    const closestUnit = sortedUnits.find((unit) => unit.id === closestUnitId);
    return closestUnit?.region ?? null;
  }, [closestUnitId, sortedUnits]);

  const groupedRegions = useMemo<RegionGroup[]>(() => {
    const regionMap = new Map<
      string,
      { region: string; units: UnitInfo[]; minDistance: number; firstIndex: number }
    >();

    sortedUnits.forEach((unit, index) => {
      const region = unit.region ?? 'Outros';
      const distance = unit.distance ?? Infinity;
    ...
    const resultWithIndex = Array.from(regionMap.values());

    resultWithIndex.sort((a, b) => {
      const aHasDistance = Number.isFinite(a.minDistance);
      const bHasDistance = Number.isFinite(b.minDistance);
    ...
    return resultWithIndex.map(({ firstIndex, ...rest }) => rest);
  }, [sortedUnits]);
          ...
          <div className="relative z-10 flex flex-col h-full px-6 py-8 md:px-10 md:py-12">
            ...
            <div className="flex-1 overflow-y-auto pb-6">
              <div className="space-y-8 max-w-6xl mx-auto">
                {groupedRegions.map((regionGroup) => {
                  const isClosestRegion = regionGroup.region === closestRegionName;
                  const hasRegionDistance = Number.isFinite(regionGroup.minDistance);
                  const regionDistanceLabel = hasRegionDistance
                    ? `Aprox. ${regionGroup.minDistance.toFixed(1)} km de você`
                    : null;
                  ...
                  return (
                    <div
                      ...
                      className={`relative overflow-hidden rounded-3xl transition-all duration-500 ease-out
                                ${
                                  isClosestRegion
                                    ? 'bg-gradient-to-br from-blue-500/20 via-blue-600/15 to-teal-500/20 backdrop-blur-xl border-2 border-blue-400/30 shadow-2xl shadow-blue-500/20'
                                    : 'bg-white/10 backdrop-blur-xl border border-white/15 shadow-lg hover:bg-white/15 hover:border-white/20'
                                }`}
                    >
                      {/* Header da região */}
                      <div className={`px-6 py-5 border-b ${isClosestRegion ? 'border-blue-400/20' : 'border-white/10'}`}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className={`flex items-center justify-center h-14 w-14 rounded-2xl ${
                              isClosestRegion 
                                ? 'bg-gradient-to-br from-blue-500 to-teal-500 shadow-lg shadow-blue-500/30' 
                                : 'bg-white/10 border border-white/20'
                            }`}>
                              <Building className={`h-7 w-7 ${isClosestRegion ? 'text-white' : 'text-blue-300'}`} />
                            </div>
                            <div>
                              <h2 className={`text-2xl font-bold text-white tracking-tight ${
                                isClosestRegion ? 'text-blue-100' : ''
                              }`}>
                                {regionGroup.region}
                              </h2>
                              {regionDistanceLabel && (
                          ...
                          {isClosestRegion && (
                            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-blue-500/30 to-teal-500/30 backdrop-blur-sm border border-blue-400/40 shadow-lg">
                              <Star className="h-4 w-4 text-yellow-300" fill="#fbbf24" />
                              <span className="text-sm text-blue-100 font-semibold">{t('closest')}</span>
                            </div>
                          )}
                        </div>
                      </div>

                      {/* Lista de unidades da região */}
                      <div className="p-6">
                        <div className={`grid gap-4 ${
                          regionGroup.units.length === 1 
                            ? 'grid-cols-1 max-w-2xl' 
                            : regionGroup.units.length === 2
                            ? 'grid-cols-1 md:grid-cols-2'
                            : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'
                        }`}>
                          {regionGroup.units.map((unit) => {
                            const unitHasDistance =
                              unit.distance !== undefined && Number.isFinite(unit.distance);
                            ...
                            return (
                              <button
                                ...
                                className={`group relative flex flex-col p-5 rounded-2xl transition-all duration-300 text-left transform hover:scale-[1.02] hover:shadow-xl
                                  ${
                                    isClosestRegion
                                      ? 'bg-white/10 hover:bg-white/15 border border-white/20 hover:border-blue-400/40'
                                      : 'bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20'
                                  }`}
                              >
                                <div className="flex items-start gap-4">
                                  <div className={`flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-xl transition-all duration-300 ${
                                    isClosestRegion
                                      ? 'bg-gradient-to-br from-blue-500/40 to-teal-500/40 group-hover:from-blue-500/60 group-hover:to-teal-500/60'
                                      : 'bg-blue-600/30 group-hover:bg-blue-600/40'
                                  }`}>
                                    <MapPin className="h-6 w-6 text-blue-200" />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <p className="text-base font-medium text-white leading-relaxed mb-1">
                                      {unit.address}
                                    </p>
                                    {unitHasDistance && (
                                  ...
                                  <ArrowRight className={`h-5 w-5 flex-shrink-0 transition-transform duration-300 group-hover:translate-x-1 ${
                                    isClosestRegion ? 'text-blue-300' : 'text-blue-200/60'
                                  }`} />
                                </div>
                              </button>
                            );
